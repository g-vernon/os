...
int caps = 0; /* Shift key pressed? */
int caps_lock = 0; /* Caps lock enabled */
...
void keyboard_handler(InterruptRegisters *regs)
{
	printf("Keyboard handler triggered \n");

	char scan_code = inb(0x60) & 0x7F;
	char pressed = inb(0x60) & 0x80;
	...
	case 58:
		...
		break;
	default:
		printf("%c", caps_lock || caps ? uppercase[(int)scan_code] :
						 lowercase[(int)scan_code]);
		break;
	}
}
